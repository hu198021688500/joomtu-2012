...
/**
 * 2012-4-9 10:20:01 UTF-8
 * @package Expression package is undefined on line 4, column 15 in Templates/Scripting/PHPClass.php.
 * @version 1.0
 *
 ...
 * @since 1.0
 */
class MyCDbConnection extends CApplicationComponent {
    ...
    /**
     *
     * @var boolean 是否启用主从分离 
     */
    private $_enable_master_slave;
    private $_pdo_master;
    private $_pdo_slave;
    ...
    public function __construct($dsn = '', $username = '', $password = '') {
        // edit by hugb
        if ($this->_enable_master_slave) {
            $dsn = $this->getMyDBConn('m');
            $this->connectionString = $dsn;
            $this->username = MY_MYSQL_USER;
            $this->password = MY_MYSQL_PASS;
        } else {
            $this->connectionString = $dsn;
     ...
     * @return object 
     */
    public function getMyPdoInstance($isRead = null) {
     ...
     * @return string 
     */
    public function getMyDBConn($type = 's') {
        $db_user = MY_MYSQL_USER;
        $db_password = MY_MYSQL_PASS;
        $db_host_m = MY_MYSQL_HOST_M;
        $db_host_s = MY_MYSQL_HOST_S;
    ...
    protected function open() {
        if ($this->_pdo === null) {
            ...
            try {
                ...
                if (YII_DEBUG) {
                    throw new CDbException(Yii::t('yii', 'CDbConnection failed to open the DB connection: {error}', array('{error}' => $e->getMessage())), (int) $e->getCode(), $e->errorInfo);
                } else {
    ...
    public function getPdoInstance($query = null) {
        ...
        if ($this->_enable_master_slave) {
            ...
            return self::getMyPdoInstance($isRead);
        } else {
            return $this->_pdo;
        }
    }
    ...
    public function createCommand($query = null) {
        $this->setActive(true);
        // edit by hugb
        if ($this->_enable_master_slave && !$this->getCurrentTransaction() && self::isReadOperation($query)) {
