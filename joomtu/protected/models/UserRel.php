...
class UserRel
{
	 ...
	 * @return boolean
	 */
	public function addUserRel($type, $uid, $myUid = null)
	{
		if (!in_array($type, array_keys($this->rels))) {
			return false;
		}
		if (!is_numeric($uid)) {
			return false;
		...
		if (!$this->isHasRel($uid, $myUid, $this->rels[$type])) {
			return $this->neo4j->createRelationship($uid, $myUid, $this->rels[$type], array('key'=>$type));
		}
		return true;
	}
	...
	public function getPath($uid, $toUid)
	{
		...
		$result = $this->neo4j->findingPathsBetweenTwoNodes($uid, $toUid, $relationships, $maxDepth = 3, $algorithm = 'shortestPath');
		var_dump($result);die();
		if (!$result) {
			return $attrNames;
		}
