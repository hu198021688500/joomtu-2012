 ...
 * @copyright Copyright &copy; 2009 Shocky Han.
 ...
 * 
 * It encapsulates the excellent jsTree component and basically it is based on CTreeView widget. 
...
class CJsTree extends CWidget
{
	...
	public function init()
	{
		...
		$cs->registerScriptFile($this->baseUrl.'/_lib.js');
		$cs->registerScriptFile($this->baseUrl.'/tree_component.min.js');
		$options=$this->getClientOptions();
		$options=$options===array()?'{}' : CJavaScript::encode($options);
		$cs->registerScript('Yii.CJsTree#'.$id,"$(function () { $(\"#{$id}\").tree($options); });");
		if($this->cssFile===null)
			$cs->registerCssFile($this->baseUrl.'/tree_component.css');
	...
	protected function getClientOptions()
	{
		...
		foreach(array('selected','opened','languages','path','cookies','ui','rules','lang','callback') as $name)
		{
			if($this->$name!==null)
